...
attempt_wireless_connection(){
    ...
    read -e -r -p "\t\tPlease enter SSID of network to connect to: " SSID

    # prompt user for password
    read -e -r -p "\t\tPlease enter password for $SSID: " -s PASS

    # attempt connection to wi-fi
    if iwctl --passphrase "$PASS" station "$INTERFACE" connect "$SSID"; then
        echo -e "\t\t\tConnected to $wifi_ssid."
        return $NET_TYPE_WIRELESS
    else
        echo -e "\t\t\tFailed to connect to  $wifi_ssid."
        return $NET_ERR
    fi
}
